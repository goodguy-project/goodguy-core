...
func getEmailBody(platform string, contests []*idl.RecentContest_ContestMessage) string {
	...
	for _, contest := range contests {
		...
		c = strings.ReplaceAll(c, "****NAME****", contest.GetName())
		startTime := time.Unix(contest.GetTimestamp(), 0)
		c = strings.ReplaceAll(c, "****START****", startTime.Format("2006年01月02日 15:04:05"))
		c = strings.ReplaceAll(c, "****DURATION****", fmt.Sprintf("时长：%v分钟", contest.GetDuration()/60))
		c = strings.ReplaceAll(c, "****URL****", contest.GetUrl())
		inner += c
	}
...
func doEmailSubscribe(m *model.Member, platform string, contest []*idl.RecentContest_ContestMessage) error {
...
func newEmailSubscribeJob(m *model.Member, contest *model.Contest) {
	...
	if now.Before(t) && t.Before(now.Add(36*time.Hour)) {
		...
		if subscribeLog.Sid != "" {
			subscribeLog.ContestMsg.Contest = append(subscribeLog.ContestMsg.Contest, contest.ContestMessage)
			db.Save(subscribeLog)
			return
		}
		model.GetDB().Create(&model.SubscribeLog{
			...
			ContestMsg: &model.ContestMsg{
				Contest:  []*idl.RecentContest_ContestMessage{contest.ContestMessage},
				Member:   m,
